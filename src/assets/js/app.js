...
data.filter(item => item.ShowTypeDesc === 'NutraIngredients').map(item => buildCard(item));
...
let eventList = $('input[name="ShowKey"]');
let selected;

eventList.change(checkBoxChange);

$('input[name="selectAll"]').change(function() {
  let eventListArray = eventList.toArray();
  if (this.checked) {
    eventListArray.map(item => item.checked = true)
  } else {
    eventListArray.map(item => item.checked = false)
  }
  selectedState();
});
